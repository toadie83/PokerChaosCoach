...
export default function LearningLibraryPage() {
  ...
  useEffect(() => {
    setLearningPageMeta({
      ...
      description: "Browse tournament poker Quick Lessons, articles, and guides used by Playback Poker Study Spots.",
      path: "/learn",
    });
  }, []);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((loadError) => {
        if (cancelled) return;
        setError(loadError?.message || "The Learning Library could not be loaded.");
        setStatus("error");
      });
    return () => {
      cancelled = true;
  ...
  return (
    <MarketingSiteShell currentPath="/learn">
      ...
      {status === "loading" ? <p className="learning-state">Loading lessons...</p> : null}
      {status === "error" ? <p className="learning-state learning-state--error">{error}</p> : null}
      ...
      {status === "ready" ? groups.map(([group, items]) => (
            ...
            {items.map((resource) => (
              ...
              >
                ...
                <div className="learning-card-tags">
                  {[resource.primaryTag, ...(resource.secondaryTags || []).slice(0, 2)].filter(Boolean).map((tag) => <span key={tag}>{learningLabel(tag)}</span>)}
                </div>
                <a className="learning-card-action" href={resource.canonicalPath}>Open lesson</a>
              </article>
            ))}
          </div>
        </section>
